...
class AgentState(TypedDict):
    ...
    # 'dashboard' key now holds the structured dict
...
"""
            try:
                # Simulating LLM call for mitigation to ensure non-Groq execution works
                if dept == "Database Operations":
                    mitigation_text = f"**Resolution Summary:** Historically, database issues were often resolved by checking the user authentication layer.\n\n**Actionable Mitigation:** Immediately run a health check on the primary user database connection pool. If risk is > 0.5, schedule a rolling restart of all database servers."
                else:
                    mitigation_text = f"**Resolution Summary:** Past similar risks were mitigated by simple service restarts and cache clearing.\n\n**Actionable Mitigation:** Verify system logs for high CPU or memory usage. Prepare to escalate to L3 if incidents double within 2 hours."
...
# ------------------------
# NODE 8: Executive Dashboard (Frontend-Ready)
# ------------------------
def executive_dashboard(state: AgentState):
    ...
    # Data structure for direct JSON serialization to the frontend
    dashboard_output = {
        "timestamp": datetime.now().isoformat(),
        "total_revenue_exposure": f"₹{total_rev_loss:,.0f}",
        "department_risks": [],
        "dependency_graph": state["dynamic_dependency_graph"],
        "summary_markdown": "" 
    }
    ...
    lines.append(f"**Total Revenue Exposure (Last 30 Days):** {dashboard_output['total_revenue_exposure']}\n")
    lines.append("---")

    for dept in DEPARTMENTS:
        risk = state["predictions"].get(dept, 0)
        # Revenue loss from *unresolved* tickets for this department
        ...
        # Structured data for the frontend table/chart
        dashboard_output["department_risks"].append({
            "department": dept,
            "risk_probability": round(risk, 4),
            "risk_level": risk_level,
            "unresolved_revenue_exposure": round(rev_loss, 2), 
            "unresolved_revenue_exposure_display": f"₹{rev_loss:,.0f}",
            "mitigation_steps": steps,
        })

        # Markdown summary string
        lines.append(f"""
### {risk_emoji} {risk_level}: {dept}
...
> {steps.replace('\n', '\n> ')}
    ...
    dashboard_output["summary_markdown"] = "\n".join(lines)
    
    # Return the structured dictionary in the 'dashboard' state key
    return {"dashboard": dashboard_output}
...
workflow.add_node("dashboards", executive_dashboard)
...
workflow.add_edge("mitigation_rag", "dashboards")
workflow.add_edge("dashboards", END)

# COMPILE: Renamed to APP_GRAPH to fix the ImportError
APP_GRAPH = workflow.compile()

# ------------------------
# EXECUTION (Frontend Output Simulation)
# ------------------------
if __name__ == "__main__":
    initial_state = {
        ...
        "predictions": {}, 
        "mitigation_steps": {}, 
        "dashboard": {}, # Initialize dashboard as empty dict
    ...
    try:
        ...
        # Retrieve the final structured data
        final_dashboard_data = result["dashboard"]

        # 💥 This is the final, structured output that a FastAPI endpoint would return
        print("\n" + "="*70)
        print("FRONTEND-READY JSON OUTPUT (LangGraph Final State: result['dashboard'])")
        print("="*70 + "\n")
        print(json.dumps(final_dashboard_data, indent=4))
        print("\n" + "="*70)

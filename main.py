...
# # CONFIG & SETUP
# # ------------------------
# # NOTE: In a production environment, load these from environment variables!
# SERVICENOW_INSTANCE = "https://dev319024.service-now.com"
# USERNAME = "admin"
# PASSWORD = "/r+j7u8eVVUQ"
...
#     "HR Systems Team",
#     "Identity & Access Mgmt" # Added for structural completeness
# ]

# # HARDCODED DEPENDENCY_GRAPH (Used as fallback if dynamic learning fails)
# DEPENDENCY_GRAPH = {
...
#     department: str
#     severity: str
#     business_impact: str # Raw text/value from API
#     resolved: bool
#     created_at: str
#     description: str
#     revenue_loss: float
...
#     # 🆕 New state key for the dynamically learned graph
...
#     # Note: Using the GLOBAL DEPT_TO_IDX and NUM_DEPARTMENTS
...
#     """
#     Engineers a cascade probability label (0.0 to 1.0) for GNN training.
#     """
...
#     """Groups historical ticket data into GNN training samples (Features H, Labels Y)."""
...
#     """Initializes and trains the GNN model."""
#     if not training_data:
#         print("   -> Insufficient data for training. Initializing GNN with random weights.")
...
#     print(f"   -> Training GNN for {NUM_EPOCHS} epochs on {len(training_data)} samples...")
...
#     print(f"   -> GNN training complete. Final Loss: {avg_loss:.4f}")
#     torch.save(model.state_dict(), GNN_WEIGHTS_FILE)
#     print(f"   -> Trained weights saved to {GNN_WEIGHTS_FILE}")
#     return model

# # ----------------------------------------------------
# # 🆕 NEW NODE: Dynamic Dependency Learning
# # ----------------------------------------------------
...
#     """Dynamically learns the dependency weights based on temporal correlation."""
#     print(f"\n🧠 Starting dynamic dependency learning (Causal Window: {causal_window_hours}h)...")
    
#     if not tickets:
#         print("   -> No tickets provided. Returning empty graph.")
#         return {}

#     df = pd.DataFrame(tickets)
#     df['created_at'] = pd.to_datetime(df['created_at'])
    
#     # Filter for only high-priority/high-impact incidents in Department B (the effect)
...
#             if sequence_count > 0:
#                 # W_A->B = (Sequences A->B) / (Total Incidents in A)
#                 weight = sequence_count / total_incidents_A
#                 # Max weight of 1.0, minimum threshold 0.1
#                 downstream_dependencies[dept_B] = round(min(max(weight, 0.1), 1.0), 4)

#         if downstream_dependencies:
#             new_dependency_graph[dept_A] = downstream_dependencies
...
#     """LangGraph node to compute the dynamic graph and store it in state."""
...
#     """Retrieves and summarizes relevant past incident close notes for a department."""
#     relevant_notes = [
...
#         # Fallback data generation (Increased density and assigned to specific past dates)
#         raw_tickets = []
#         for i in range(2000): # Increased ticket count for training data density
...
#                 # Conditional assignment_group: empty string triggers LLM
...
#     return {"tickets": mapped, "historical_context": {}, "dynamic_dependency_graph": DEPENDENCY_GRAPH} # Initialize with static graph
...
#     # ... (LLM Triage logic remains the same) ...
#     for ticket in tickets:
#         current_dept = ticket["department"]
        
#         if current_dept in UNASSIGNED_CRITERIA:
#             print(f"   -> Unassigned INC {ticket['id']}. Running LLM triage...")
            
#             prompt = f"""
#             Analyze the following incident description: "{ticket['description']}"
            
#             Determine the most appropriate owner from these available departments: {', '.join(departments_list)}.
            
#             Respond with ONLY the name of the most appropriate department. DO NOT provide any other text, explanation, or punctuation.
#             """
            
#             try:
#                 response = llm.invoke(prompt)
#                 classified_dept = response.content.strip()
                
#                 if classified_dept in departments_list:
#                     print(f"   -> Classified INC {ticket['id']}: **{classified_dept}**")
#                     ticket["department"] = classified_dept
#                     classified_count += 1
#             except Exception as e:
#                 print(f"❌ LLM Classification failed for ticket {ticket['id']}: {e}")
    
#     if classified_count == 0:
#         print("✅ All tickets were already assigned or could not be classified.")
    
#     # Retrieve RAG context using the finalized department assignments
...
#     # 💥 CRITICAL CHANGE: Use the dynamic graph from the state
...
#     # DYNAMIC WEIGHTS LOGIC: Load trained weights or train the model
#     if os.path.exists(GNN_WEIGHTS_FILE):
#         print(f"   -> Loading trained GNN weights from {GNN_WEIGHTS_FILE}")
...
#         # 💥 CRITICAL CHANGE: Pass the dynamic adj_matrix to the training function
#         gnn_model = train_gnn_model(training_data, adj_matrix) 
    
#     # 1. Prepare Node Features (H) from current LSTM forecast
...
# Based on the RAG CONTEXT, especially the **PAST RESOLUTION NOTES**, and the forecasted RISKS, provide two things:
# 1. **Resolution Summary:** A brief summary (2-3 sentences) of *how similar high-risk issues were historically resolved* in this department.
# 2. **Actionable Mitigation:** Detailed, proactive steps the {dept} team should execute immediately to prevent the forecasted cascade, referencing the historical context where applicable.
# """
#             try:
#                 resp = llm.invoke(prompt)
#                 mitigation_steps[dept] = resp.content.strip()
#             except Exception as e:
...
#     lines.append("## 🚨 Proactive Incident Forecast Summary\n")
...
# * **Mitigation Steps (LLM Generated):**
# > {steps.replace('\n', '\n> ')}
#         """)
...
# # 🆕 INSERTED NODE for dynamic graph learning
...
# # 🆕 NEW EDGE: LSTM -> Dynamic Dependency Learning
# workflow.add_edge("lstm_forecast", "dependency_learning") 
# # 🆕 NEW EDGE: Dynamic Dependency Learning -> GNN
...
#     print("\n" + "="*70)
#     print("PROACTIVE INCIDENT AGENT (DYNAMIC DEPENDENCY + GNN) STARTED")
#     print("="*70 + "\n")
...
#         print("!! LLM steps (triage, mitigation) will likely fail.         !!")
#         print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
#         print("\n" + "="*70)
#         print("RISK PREDICTIONS (GNN Output)")
#         print("="*70)
#         for dept, prob in result["predictions"].items():
#             if not isinstance(prob, np.ndarray):
#                 print(f"{dept:<30}: {prob:.4f}")
        
#         print("\n" + "="*70)
#         print("DYNAMIC DEPENDENCY GRAPH (Learned Structure)")
#         print("="*70)
#         # Use json.dumps for clean, readable output of the learned structure
#         print(json.dumps(result["dynamic_dependency_graph"], indent=4))
        
#     except Exception as e:
#         print(f"\n❌ An error occurred during execution: {e}")
#         print("Please ensure all dependencies are installed and the API key is valid.")